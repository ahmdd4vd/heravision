...
FEATURES = ["area_ratio", "aspect_ratio", "compactness", "solidity", "boundary_strength", "scale_stability", "color0", "color1", "color2", "texture0", "crop_luma_mean", "crop_luma_std", "crop_chroma_r_mean", "crop_chroma_b_mean", "crop_chroma_mag", "crop_edge_density", "crop_dark_fraction", "crop_bright_fraction", "x", "y", "w", "h"]
...
def node_features(node, width, height, arr):
    r=node["region"]; f=r.get("features",{}); b=r["bbox"]; color=f.get("color",[0.0]); texture=f.get("texture",[0.0]); crop=crop_features(arr,b)
    return [float(f.get("area_ratio",0)), min(1.0,float(f.get("aspect_ratio",0))/8), min(1.0,float(f.get("compactness",0))), min(1.0,float(f.get("solidity",0))), float(f.get("boundary_strength",0)), float(f.get("scale_stability",0)), min(1.0,(float(color[0])+4)/8), min(1.0,(float(color[1])+4)/8), min(1.0,(float(color[2])+4)/8), min(1.0,abs(float(texture[0]))*20), *crop, b["x"]/max(1,width), b["y"]/max(1,height), b["w"]/max(1,width), b["h"]/max(1,height)]
